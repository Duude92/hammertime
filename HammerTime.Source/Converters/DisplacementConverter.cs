...
namespace HammerTime.Source.Converters
{
	[Export(typeof(IMapObjectSceneConverter))]
	public class DisplacementConverter : IMapObjectSceneConverter
	{
		...
		{
			return obj.Data.OfType<Displacement>().Any();
		}

		public Task Convert(BufferBuilder builder, MapDocument document, IMapObject obj, ResourceCollector resourceCollector)
		{
			return ConvertFaces(builder, document, obj, obj.Data.Get<Displacement>().First(), resourceCollector);
		}
		internal static async Task ConvertFaces(BufferBuilder builder, MapDocument document, IMapObject obj, Displacement face, ResourceCollector resourceCollector)
		{
			...
			var numVertices = (uint)face.Vertices.Count;

			// Pack the indices like this [ solid1 ... solidn ] [ wireframe1 ... wireframe n ]

			uint numSolidIndices = (uint)(face.Vertices.Count - 1) * 4;
			uint numWireframeIndices = numVertices * 6;
			...
			var f = face;
			{
				uint texInd = (uint)(f.Vertices.Count - 1) * 4;
