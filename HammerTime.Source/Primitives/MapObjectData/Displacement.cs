...
namespace HammerTime.Source.Primitives.MapObjectData
{
	public class Displacement : IMapObjectData, ITransformable, ITextured
	{
		...
		public class ActiveTextureFormatter : StandardMapElementFormatter<Displacement> { }
		...
		private void CopyBase(Displacement face)
		{
		...
		{
			var face = new Displacement(numberGenerator.Next("Displacement"));
			CopyBase(face);
			return face;
		}
		...
		{
			var so = new SerialisedObject("Displacement");
			so.Set("ID", ID);
