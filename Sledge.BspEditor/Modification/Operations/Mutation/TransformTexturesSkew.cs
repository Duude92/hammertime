...
namespace Sledge.BspEditor.Modification.Operations.Mutation
{
	...
	/// </summary>
	public class TransformTexturesSkew : IOperation
	{
		...
		public TransformTexturesSkew(Matrix4x4 matrix, params IMapObject[] objectsToTransform)
		{
			_matrix = matrix;
			_idsToTransform = objectsToTransform.Select(x => x.ID).ToList();
		}

		public TransformTexturesSkew(Matrix4x4 matrix, IEnumerable<IMapObject> objectsToTransform)
		{
			_matrix = matrix;
			_idsToTransform = objectsToTransform.Select(x => x.ID).ToList();
		}
		...
		public Task<Change> Perform(MapDocument document)
		{
			...
			foreach (var o in objects)
			{
				...
				{
					it.Texture.TransformSkew(_matrix);
					ch.Update(o);
				}
			}
