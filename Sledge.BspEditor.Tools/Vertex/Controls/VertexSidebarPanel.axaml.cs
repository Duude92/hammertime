...
public partial class VertexSidebarPanel : UserControl, ISidebarComponent, IManualTranslate
{
	...
	public void Translate(ITranslationStringProvider strings)
	{
		...
		this.InvokeLater(() =>
		{
			...
			DeselectAllButton.Content = strings.GetString(prefix, "DeselectAll");
			ResetButton.Content = strings.GetString(prefix, "ResetToOriginal");
		});
	}
	...
	public void SetSelectedTool(Type tool)
	{
		...
		foreach (RadioButton rb in ButtonLayoutPanel.Children)
		{
			if (rb.Tag as Type != tool) rb.IsChecked = false;
			else if (!rb.IsChecked.Value) rb.IsChecked = true;
		}
		...
		ControlPanel.Content = t.Title;
		if (t.Control != null)
		{
			//ControlPanel.Controls.Add(t.Control);
			ControlPanel.Content = t.Control;
	...
	{
		if (ButtonLayoutPanel.Children.Count > 0) return;
	...
	private void AddSubTool(VertexSubtool tool)
	{
		...
		if (ButtonLayoutPanel.Children.Count == 0) rdo.IsChecked = true;
		ButtonLayoutPanel.Children.Add(rdo);
