	...
	public class NativeFile : IFile
	{
		...
		public IEnumerable<IFile> GetChildren()
		{
			...
			foreach (var d in children)
			{
				...
				{
					var file = dirs.First(x => String.Equals(x.Name, d.Name, StringComparison.CurrentCultureIgnoreCase));
					dirs.Remove(file);
					IFile newFile = null;
					if (file.Type != FileSystemType.Composite)
					{
						var parent = file is InlinePackageFile ? null : file.Parent;
						newFile = new CompositeFile(parent, new List<IFile>() { file, d });
					}
					else
					{
						var composite = file as CompositeFile;
						var oldFIles = composite.GetFiles().ToList();
						oldFIles.Add(d);
						newFile = new CompositeFile(file.Parent, oldFIles);
					}
					dirs.Add(newFile);
				}
			}
