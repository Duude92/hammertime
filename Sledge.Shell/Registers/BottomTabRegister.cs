...
using static Sledge.Shell.ControlExtensionsAv;
...
namespace Sledge.Shell.Registers
{
	...
	public class BottomTabRegister : IStartupHook, IInitialiseHook
	{
		...
		private readonly Lazy<Forms.ShellAv> _shell;
		...
		internal BottomTabRegister(
			[Import] Lazy<Forms.ShellAv> shell,
		...
		public Task OnInitialise()
		{
			...
			{
				foreach (var tab in _shell.Value.BottomTabs.Items.OfType<TabItem>())
				{
					if (!(tab.Tag is IBottomTabComponent btc)) continue;
					tab.Header = btc.Title;
				}
			});
		...
		{
			_shell.Value.Invoke(delegate
			{
				_shell.Value.BottomTabs.Items.Clear();
				foreach (var btc in _components)
				{
					var page = new TabItem { Tag = btc, IsVisible = false, Header = btc.Title, MinWidth = 50 };
					page.Content = ((Control)btc.Control);
					_shell.Value.BottomTabs.Items.Add(page);
				}
			});
		}
		...
		private Task ContextChanged(IContext context)
		{
			_shell.Value.Invoke(delegate
			{
				...
				foreach (var tab in _shell.Value.BottomTabs.Items.OfType<TabItem>())
				{
					...
					var vis = tab.IsVisible;
					tab.Header = btc.Title;

					if (iic != vis) tab.IsVisible = iic;
				}
