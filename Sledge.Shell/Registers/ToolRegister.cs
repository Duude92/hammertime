	...
	public class ToolRegister : IStartupHook, IHotkeyProvider
	{
		...
		private async Task ContextChanged(IContext context)
		{
			...
			_shell.Invoke(delegate
			{
				...
				_shell.ToolsContainer.Children.Clear();
				foreach (var tl in toolsInContext)
				{

					object icon = tl.Name;

					if (!string.IsNullOrEmpty(tl.ImageName))
					{

						var uri = new Uri($"avares://{tl.Namespace}/Resources/" + tl.ImageName + ".png");
						try
						{
					...
					{
						Content = icon is string ? icon : new Image { Source = (Bitmap)icon },
						Name = tl.Name,
						//IsChecked = tl == activeTool,

						Padding = new Thickness(0),
						Width = 36,
						Height = 36,
					};
					//TODO: Implement separate Button with checked class and tooltip
					if (tl == activeTool)
						toolButton.Classes.Add("checked");
					else
						toolButton.Classes.Remove("checked");
					toolButton.Click += async (s, ea) => await ActivateTool(tl);
					...
					//    Height = 36
					//};
					_shell.ToolsContainer.Children.Add(toolButton);
				}
