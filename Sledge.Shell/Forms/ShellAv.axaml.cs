...
namespace Sledge.Shell.Forms
{
	...
	[AutoTranslate]
	internal partial class ShellAv : Window, ISettingsContainer
	{
		...
		private void InitializeShell()
		{
			...
			Oy.Subscribe<DocumentCloseMessage>("Document:RequestClose", async d => await Dispatcher.UIThread.InvokeAsync(() => DocumentRequestClose(d)));

			Oy.Subscribe<string>("Shell:OpenCommandBox", async o => await Dispatcher.UIThread.InvokeAsync(() => OpenCommandBox(o)));
		...
		private async void DoClosing(object sender, CancelEventArgs e)
		{
			...
			//Dispatcher.UIThread.InvokeSync(() => {
		...
		//internal IEnumerable<DockedPanelAv> GetDockPanels()
		//{
		...
		{
			var tab = DocumentTabs.Items.OfType<TabItem>().FirstOrDefault(x => x.Tag == document);
			if (tab != null)
			{
				var idx = DocumentTabs.Items.IndexOf(tab);
			...
			{
				var currentControl = DocumentContainer.Children.OfType<Avalonia.Controls.Control>().FirstOrDefault();
		...
		{
			var page = DocumentTabs.Items.OfType<TabItem>().FirstOrDefault(x => x.Tag == document);
			if (page != null) DocumentTabs.Items.Remove(page);
			return Task.CompletedTask;
		}
