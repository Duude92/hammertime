...
namespace Sledge.BspEditor.Editing.Components.Properties.Tabs
{
	...
	public sealed partial class VisgroupTab : UserControl, IObjectPropertyEditorTab
	{
		...
		private bool _objectColorChanged;
		...
		/// <inheritdoc />
		public bool HasChanges => GetMembershipChanges().Count > 0 || _objectColorChanged;
		...
		public async Task SetObjects(MapDocument document, List<IMapObject> objects)
		{
			visgroupPanel.InvokeLater(() =>
			{
				...
				PaintObjectColor(objects.FirstOrDefault()?.Data.Get<ObjectColor>().FirstOrDefault()?.Color ?? Colour.GetRandomBrushColour());
		...
		public IEnumerable<IOperation> GetChanges(MapDocument document, List<IMapObject> objects)
		{
			...
			if (_objectColorChanged)
			{
				foreach (var mo in objects)
				{

					var objColor = objectColorPanel.BackColor;
					var oldColor = mo.Data.Get<ObjectColor>();
					yield return new Transaction(
						new RemoveMapObjectData(mo.ID, oldColor),
						new AddMapObjectData(mo.ID, new ObjectColor(objColor))
						);
				}
			}
		}
		...
		private void PaintObjectColor(Color color)
		{
			_objectColorChanged = false;

			objectColorPanel.BackColor = color;
		}
		private void ObjectColorPanel_Click(object sender, EventArgs e)
		{
			using (var cp = new ColorDialog { Color = objectColorPanel.BackColor })
			{
				if (cp.ShowDialog() != DialogResult.OK) return;

				objectColorPanel.BackColor = cp.Color;
				_objectColorChanged = true;
			}
		}
	}
}
