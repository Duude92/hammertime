...
public partial class VisgroupPanel : UserControl
{
	...
	{
		get => StateImageList != null;
		set => StateImageList = value ? CheckboxImages : null;
	}
	private ListBox StateImageList { get; set; }
	...
	{
		get => (VisgroupTree.SelectedItem as TreeNode)?.Tag as VisgroupItem;
		set => VisgroupTree.SelectedItem = GetNodeForTag(value?.Tag);
	}
	...
	//	return bmp;
	//}
	private class TreeNode : TreeViewItem
	{
	...
	private void AddNode(TreeNode parent, List<VisgroupItem> visgroups, VisgroupItem visgroup)
	{
		...
		{
			Header = visgroup.Text,
			StateImageKey = CastCheckState(visgroup.CheckState) + (visgroup.Disabled ? "Disabled" : ""),
			// TODO: Should I use do it sonce Avalonia has themes?
			Background = visgroup.Colour.ToBrush(),
			Foreground = visgroup.Colour.GetIdealForegroundColour().ToBrush(),
			Tag = visgroup
		};

		if (parent == null) VisgroupTree.Items.Add(node);
		else parent.Items.Add(node);
	...
	{
		return GetAllNodePaths(VisgroupTree.Items.OfType<TreeNode>().ToDictionary(x => x, x => x.Header as string));
	}
	...
	private Dictionary<TreeNode, string> GetAllNodePaths(Dictionary<TreeNode, string> nodes)
	{
		...
		var children = nodes.SelectMany(x => x.Key.Items.OfType<TreeNode>().Select(n => new { x, n }))
			.ToDictionary(x => x.n, x => x.x.Value + "/" + x.n.Header);
		if (children.Any())
		{
	...
	{
		return GetAllNodes(VisgroupTree.Items.OfType<TreeNode>());
	}

	private IEnumerable<TreeNode> GetAllNodes(IEnumerable<TreeNode> nodes)
	{
		var n = nodes.ToList();
		return n.SelectMany(x => GetAllNodes(x.Items.OfType<TreeNode>())).Union(n);
	}
	...
	public void UpdateVisgroupState(VisgroupItem visgroup)
	{
		...
		node.Header = visgroup.Text;
		node.StateImageKey = CastCheckState(visgroup.CheckState);
		node.Background = visgroup.Colour.ToBrush();
		node.Foreground = visgroup.Colour.GetIdealForegroundColour().ToBrush();
	...
	private void NodeMouseClick(object sender, PointerPressedEventArgs ev)
	{
		if (!ShowCheckboxes) return;
		if (sender is TreeView tree && tree.SelectedItem is TreeNode node)
		{


			var e = ev.GetPosition(VisgroupTree);

			// Only do something if the click is over the image (checkbox)
			var hit = VisgroupTree.InputHitTest(e);
			//TODO: check this
			...
			var vis = (VisgroupItem)node.Tag;

			// unchecked -> checked, checked -> unchecked, mixed -> unchecked
			var visible = node.StateImageKey.StartsWith("Unchecked");
			node.StateImageKey = (visible ? "Checked" : "Unchecked");
			OnVisgroupToggled(vis, visible ? CheckState.Checked : CheckState.Unchecked);
		}
	}

	private void NodeSelected(object sender, SelectionChangedEventArgs e)
	{
